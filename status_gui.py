...
class TitanStatusApp:
    def __init__(self, root):
        ...
        self.pause_btn = tk.Button(self.btn_frame, text='PAUSE CURRENT LOOP', font=('Inter', 10, 'bold'), bg='#ff8a00', fg='white', command=self.pause_loop)
        ...
        self.log_box = tk.Text(root, height=10, bg='#050508', fg='#00ff47', font=('Consolas', 10), borderwidth=1, relief='solid', padx=10, pady=10)
        self.log_box.pack(padx=50, pady=5, fill='both', expand=True)

        self.footer = tk.Label(root, text='WEB CLIENT IS MASTER. PILOOP AWAITS COMMANDS.', font=('Inter', 10), bg='#0a0b10', fg='#a0a0a0', pady=10)
        self.footer.pack()
    ...
    def clear_loop(self):
        try:
            ...
            self.add_log('?? Failed to contact bridge for CLEAR')
            
    def pause_loop(self):
        try:
            urllib.request.urlopen('http://127.0.0.1:5000/pause', timeout=2)
        except:
            self.add_log('?? Failed to contact bridge for PAUSE')
